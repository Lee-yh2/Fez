...
HRESULT CUIkey::Ready_GameObject_Prototype()
{
...
HRESULT CUIkey::Ready_GameObject(void * pArg)
{
	...
	if (FAILED(Add_Component(ePrototypeType::Static, L"Component_Texture_UIkey", L"Com_Cube", (CComponent**)&m_pTextureCom)))
	{
...
_uint CUIkey::Update_GameObject(_float fDeltaTime)
{
	CManagement*	pManagement = CManagement::Get_Instance();
	POINT Std = { (LONG)30.f, (LONG)90.f };
...
_uint CUIkey::Late_Update_GameObject(_float fDeltaTime)
{
...
HRESULT CUIkey::Render_GameObject()
{
...
CUIkey * CUIkey::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CUIkey*	pInstance = new CUIkey(pDevice);
...
CGameObject * CUIkey::Clone(void * pArg)
{
	CUIkey*	pClone = new CUIkey(*this);
...
HRESULT CUIkey::Movement(_float fDeltaTime)
{
	return S_OK;
}

_vec3 CUIkey::change_view(POINT _pt, _uint _Wincx, _uint _Wincy, CCamera* pCamera, _vec4x4* pWorldMatrix)
{
